package fuzzer.adapters.pandas
...
class PandasCodeGenerator(config: FuzzerConfig, spec: JsValue, dag2CodeFunc: Graph[DFOperator] => SourceCode) extends CodeGenerator {

  override def getDag2CodeFunc: Graph[DFOperator] => SourceCode = dag2CodeFunc
}

class PandasDataAdapter(config: FuzzerConfig) extends DataAdapter {
  ...
  override def getTables: Seq[TableMetadata] = {
    try {
      ...
      val response = HttpUtils.postJson(client, requestJson, host = "localhost", port = 8891)
  ...
  override def loadData(executor: CodeExecutor, filterF: String => Boolean = _ => true): Unit = {
    ...
    val response = HttpUtils.postJson(client, request, "localhost", 8891, timeoutSeconds = 120)

    val body = Json.parse(response.body())
    val success = (body \ "success").asOpt[Boolean]
    success match {
      case Some(true) =>
      case _ => throw new Exception("Pandas server failed to load data!")
    }
  }
...
class PandasCodeExecutor(config: FuzzerConfig, spec: JsValue) extends CodeExecutor {
  ...
  override def setupEnvironment(): () => Unit = {
    val processBuilder = new java.lang.ProcessBuilder(
      ...
      "oracle-servers/pandas-oracle-server/basic-json-server.py",
      "--out-dir", config.outDir,
      s"--${if (config.coverageCaptureOn) "" else "no-"}coverage-capture",
    )
    processBuilder.redirectOutput(new File("oracle-servers/.logs/pandas-server.log"))
