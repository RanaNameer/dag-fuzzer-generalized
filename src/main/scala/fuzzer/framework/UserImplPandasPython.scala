...
object UserImplPandasPython {
  ...
  def dag2PandasPython(spec: JsValue)(graph: Graph[DFOperator]): SourceCode = {
    ...
    // pandas is eager - there's no lazy plan to trigger, this just gives the oracle
    // server something informative in stdout to log per execution.
    l += s"""print($finalVariableName.shape)"""
  ...
  def generatePreloadedUDF(): String = {
    ...
    val outDir = s"generated/Pandas"
    val outPath = s"$outDir/udfs_$batch.json"
    val prompt = s"""
Generate a json file of the following format
```
{ "functions": ["def dfPipeUdf(df): ..."] }
```
...
""".trim
  ...
  private def generateSourceOperation(
    ...
    opName match {
      case "pd.read_parquet" =>
        s"""pd.read_parquet("tpcds-data-5pc/$tableName").rename(columns={col: f"{col}_${node.id}" for col in pd.read_parquet("tpcds-data-5pc/$tableName").columns})"""
      case _ =>
        // Generate synthetic data as a plain pandas DataFrame
  ...
  private def generateUnaryOperation(
    ...
    opName match {
      ...
      case "pipe" => generatePipeOperation(node, parameters, in1)
  ...
  private def generatePipeOperation(
                                     node: Node[DFOperator],
                                     parameters: JsObject,
                                     in1: String
                                   ): String = {
    // pandas has no partitions - a "map over the data" operator is just a whole-DataFrame
    // transform via .pipe(), either the preloaded UDF or a small inline lambda.
    val (_, col) = pickRandomColumnFromReachableSources(node)
    val colName = col.name

    val udfProb = 0.3
    if (Random.nextDouble() < udfProb) {
      s"$in1.pipe(dfPipeUdf)"
    } else {
      col.dataType match {
        case fuzzer.data.types.IntegerType | fuzzer.data.types.LongType =>
          s"$in1.pipe(lambda d: d.assign($colName=d['$colName'] * 2))"
        case fuzzer.data.types.FloatType | fuzzer.data.types.DecimalType =>
          s"$in1.pipe(lambda d: d.assign($colName=d['$colName'].round(2)))"
        case fuzzer.data.types.StringType =>
          s"$in1.pipe(lambda d: d.assign($colName=d['$colName'].str.upper()))"
        case _ =>
          s"$in1.pipe(lambda d: d)"
      }
    }
  }
  ...
  private def generateHeadOperation(
    ...
    // no compute= kwarg here - unlike dask, pandas .head() is already eager
  ...
  // NOTE: "column-rename" is intentionally left as a no-op here, matching the same
  // not-yet-implemented gap in UserImplDaskPython/UserImplPolarsPython - generateRenameOperation
  // emits a real .rename() in the generated program, but stateView isn't updated to match it.
